<template>
  <div class="Favorites">
    ...
    <div class="FavoritesList">
      <div v-if="flag === true" class="favorites">
        <div
...
export default {
  ...
  methods: {
    ...
    getMyFavorites() {
      ...
      api.favorites.getMyFavorites(id).then((res) => {
        if (res.status === 200) {
          this.favoritesList = [];
          res.data.forEach((item) => this.favoritesList.push(item));
          this.flag = true;
...
.Favorites {
  ...
  .FavoritesList {
    ...
    /* 隐藏滚动条上下箭头按钮 */
    .favorites::-webkit-scrollbar-button {
      display: none;
    }
